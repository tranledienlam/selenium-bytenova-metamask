...
class Auto:
    ...
    def import_wallet(self) -> bool:
        ...
        if len(self.seeds) != 12:
            self.node.snapshot(f'{len(self.seeds)}/12 seed. Số seed không hợp lệ')
            return False
    ...
    def change_network(self, network_name: str, rpc_url: str, chain_id: str, symbol: str, block_explorer_url: str = None):
        """Thay đổi mạng lưới"""
        ...
        if network_name in current_network:
            ...
            if not self.node.find_and_click(By.CSS_SELECTOR, f'[data-testid="{network_name}"]', timeout=10):
                actions_add_network = [
                    ...
                    (self.node.find_and_input, By.CSS_SELECTOR, '[data-testid="explorer-url-input"]', block_explorer_url),
                    (self.node.find_and_click, By.XPATH, '//button[text()="Add URL"]'),
                    # save network
    ...
    def _run(self):
        '''
        Automation việc import ví 12 seed và unlock ví.
        Có snapshot dừng thực thi khi thất bại.
        '''
        # Chuyển đến trang extension
        self.node.go_to(f'{EXTENSION_URL}/home.html')
