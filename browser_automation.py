...
class Node:
    def __init__(self, driver: webdriver.Chrome, profile_name: str, data_tele: tuple = None, ai_helper : AIHelper = None) -> None:
        ...
        '''
        self._driver = driver
        self._ai_helper = ai_helper  # Nhận ai_helper từ BrowserManager
    ...
    def ask_ai(self, prompt: str, is_image: bool = True, wait: int = None) -> str:
        ...
        Args:
            ...
            wait (int, optional): Thời gian chờ trước khi gửi yêu cầu đến AI, mặc định là giá trị của `self.wait = 3`.
        Returns:
            str: Kết quả phân tích từ AI. Trả về None nếu có lỗi xảy ra.
        '''
        wait = wait if wait or wait == 0 else self.wait
        if not self._ai_helper.is_working:
            return None
        ...
        if is_image:
            ...
            text, error = self._ai_helper.ask(prompt, img)
        else:   
            text, error = self._ai_helper.ask(prompt)
